...
class IPMD(OffPolicyAlgorithm):
    """
    Inverse Policy Mirror Descent (PMD)
    ...
    def _setup_model(self) -> None:
        ...
        self.expert_replay_buffer = ReplayBuffer(10000, 
                                    observation_space=self.env.observation_space,
                                    action_space=self.env.action_space,
                                    device=self.device)
        self.load_replay_buffer_to(path=self.expert_replay_buffer_loc)
        self.expert_replay_data = self.expert_replay_buffer._get_samples(np.arange(0, self.traj_size))
        # Sample expert replay buffer
        # self.expert_replay_data = copy.deepcopy(self.expert_replay_buffer._get_samples(np.arange(0, 10000)))
        # self.expert_replay_data = copy.deepcopy(self.expert_replay_buffer._get_samples(np.arange(self.expert_replay_buffer.size()-10000, self.expert_replay_buffer.size())))
        # print(self.expert_replay_data.rewards.size())
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        optimizers = [self.actor.optimizer, self.critic.optimizer, self.reward_est.optimizer]
        ...
        for gradient_step in range(gradient_steps):
            ...
            expert_data = self.expert_replay_buffer.sample(batch_size, env=self._vec_normalize_env)
            ...
            # expert_estimated_rewards = th.cat(self.reward_est(expert_data.observations, expert_data.actions), dim=1)
